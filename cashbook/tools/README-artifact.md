# 役員用ページ（claude.ai の Artifact）の更新手順

会計担当から受け取ったJSONで、役員が見るページを最新にする手順です。
**新しい会話でこのリポジトリだけを渡された場合も、これを読めば作れます。**

## まず確認：ここで公開できるか

**既存のページを更新するには、Artifact ツールに `url` を指定できる必要があります。**
claude.ai の通常のチャットにはこの指定が無く、新しいページしか作れません。

`url` を指定できない場所で頼まれたときは、**公開しないでください。**
組み立てたファイルを会計担当に返し、Claude Code（このリポジトリを開いた状態）で
仕上げてもらってください。新しいページを作ると、役員が見ているページは古いまま、
別のページだけが増えます。

## 前提

- 帳簿の中身（個人名・金額）は**このリポジトリに入っていません**。毎回、会計担当から
  受け取るJSONを使います。受け取ったJSONを**リポジトリに置かないでください**。
- 反映先のArtifactのURLは、会計担当のメッセージに書かれています。
  **書かれていない場合は、新しいページを作らずに、先に本人へURLを聞いてください。**
  新規に公開すると、役員が見ているページは古いまま別のページが増えてしまいます。
  もちろん、推測して無関係のページを上書きするのも禁止です。

## 手順

```bash
# 1. 受け取ったJSONを、リポジトリの外（作業用の場所）に置く
#    例: /tmp/backup.json

# 2. 単一ファイル版を組み立てる（--data で渡すので、リポジトリは汚れない）
cd cashbook
python3 tools/build_artifact.py --data /tmp/backup.json
```

うまくいくと、こう出ます。

```
取り込み: ○○件 / 現在残高 ○,○○○,○○○（残高欄 ○○ か所を検算）
書き出し日時: ○○○○-○○-○○ ○○:○○
saved: .../cashbook/dist/artifact.html ○○○,○○○ bytes
OK: 単一ファイルとして整合
```

- **書き出し日時**は、会計担当がアプリで書き出した時刻です（JSON内の `exportedAt`）。
  メッセージに書かれた日時と食い違う場合は、**古いファイルを渡された可能性**があるので
  確認してください。「（記載なし）」と出たら古い版で書き出されたJSONです。

- **残高が合わないJSONは、ここで止まります**（終了コード1、成果物は書き換わりません）。
  「残高が合いません」と出たら組み立てずに、会計担当に確認してください。勝手に直さないこと。
- 出てきた件数と残高を、会計担当のメッセージと照合してください。
- 各行の残高欄は2種類あり、両方を突き合わせます。
  `bookBalance`（手書きノートに書かれていた残高）と `balance`（アプリが計算した残高）です。
  アプリで追加した行は `bookBalance` を持たないので、`balance` 側で検算されます。
- 「残高欄を持たない行が○件」と出た場合、その行だけは突き合わせできていません。
  件数が多いときは会計担当に確認してください。

```bash
# 3. できた cashbook/dist/artifact.html を Artifact として公開する
#    （Artifact ツールに、会計担当から聞いたURLを url として渡す。
#     favicon は 📒、capabilities は {"downloads": true} を維持する）
```

## 公開するときの決まり

| 項目 | 値 |
|---|---|
| ファイル | `cashbook/dist/artifact.html` |
| url | 会計担当のメッセージにあるURL（**新規に作らない**。同じページを更新する）。<br>書かれていなければ公開せず、先に聞く |
| favicon | 📒 |
| capabilities | `{"downloads": true}` |

## やってはいけないこと

- 受け取ったJSONや、実データ入りの `data/ledger.js` を**コミットしない**
- `dist/artifact.html`（実データ入り）を**コミットしない**
- Artifactの**URLをこのリポジトリに書かない**
- パスワード（ログイン用・管理者用）を**平文で書かない**。`js/auth.js` はハッシュだけを持つ

## 確認したいとき

`cashbook/` には動作確認用のスクリプトは入れていません（開発一式は会計担当が
ZIPで持っています）。中身を軽く確かめるだけなら、組み立て後に次を見てください。

- 件数と残高が、上記の出力と会計担当の申告に一致しているか
- `dist/artifact.html` に `<html>` `<head>` `<body>` `<!DOCTYPE>` が無いこと
  （build_artifact.py が自動で確認し、あれば止まります）
