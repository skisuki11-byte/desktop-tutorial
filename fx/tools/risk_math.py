...
モデルの前提と限界:
  - 日次リターンは iid 正規と仮定している。この仮定は検証済み:
    t分布(df=4)やGARCHに差し替えてもDD分布はほとんど動かない
    (年次に集計する過程で中心極限定理が効き、日次の尖りは消える)。
    詳細は docs/05-検証結果.md の検証3。
  - ★ ただし、ここで出る数値は依然として「楽観側の推定」である。
    理由はファットテールではなく、シャープを確定値として扱っていること。
    OOS3年で測ったシャープ1.5の標準誤差は0.84で、95%信頼区間は
    [-0.15, 3.15] とゼロを含む。推定誤差を織り込むと年間マイナスの
    確率は8.8%->15.1%に上がる。docs/05-検証結果.md の検証4を参照。
