...
(function () {
  ...
  var store = {
    ...
    days: {}, mock: { round: 1, cleared: [] }, unlocked: false, plan: null, run: null
  };
  var session = null;
  ...
  // 問題の id（c3-07 のような形）を3文字に詰める＝章1文字＋番号2文字
  function packId(id) {
    var m = /^c(\d+)-(\d+)$/.exec(id);
    return m ? b36(+m[1], 1) + b36(+m[2], 2) : "";
  }
  function unpackId(s) {
  ...
  function packState(st) {
    ...
    for (i = 0; i < ids.length; i++) {
      var key = packId(ids[i]);
      if (!key) continue;
      var s = st.stats[ids[i]];
      if (!s || (s.c + s.w) === 0) continue;
      rec.push(key + b36(Math.min(35, s.c), 1)
    ...
    return [
      ...
      // 途中でやめた模擬試験。回・出題順・そこまでの正誤を持っておく
      st.run ? b36(st.run.round, 1) + "." + st.run.ids.map(packId).join("")
        + "." + st.run.ok.map(function (o) { return o ? "1" : "0"; }).join("") : ""
    ].join("~");
  }
  ...
  function unpackState(text) {
    ...
    for (i = 0; i + 6 <= rec.length; i += 6) {
      var streak = n36(rec.charAt(i + 5));
      stats[unpackId(rec.substr(i, 3))] =
        { c: n36(rec.charAt(i + 3)), w: n36(rec.charAt(i + 4)), run: streak, lastWrong: streak === 0 };
    }
    ...
    // 途中でやめた模擬試験
    var run = null, rp = String(f[9] || "").split(".");
    if (rp.length === 3 && rp[1]) {
      var rids = [];
      for (i = 0; i + 3 <= rp[1].length; i += 3) rids.push(unpackId(rp[1].substr(i, 3)));
      run = {
        round: n36(rp[0]) || 1, ids: rids,
        ok: String(rp[2]).split("").map(function (c) { return c === "1"; })
      };
    }
    return {
      t: n36(f[1]) * 60000,
  ...
  // fromMock のときは「今日の◯問」に数えない。模試60問で日課が終わった扱いになると、
  // その日の積み上げが見えなくなるため、別枠にしておく。
  function record(id, ok, fromMock) {
    ...
    var k = todayKey();
    if (fromMock) {
      // 模試だけやった日も「解いた日」として連続日数には入れたいので、印だけ残す
      if (store.days[k] == null) store.days[k] = 0;
    } else {
  ...
  function startMock(round) {
    ...
    if (!session) return;
    // 60問は一度に解き切れないことがあるので、出題順を控えて途中から戻れるようにする
    store.run = {
      round: round,
      ids: session.views.map(function (v) { return v.item.id; }),
      ok: []
    };
    save(true);
  }

  // 途中でやめた模試のつづき。出題順はそのまま、答えた問題は飛ばす。
  function resumeMock() {
    var run = store.run;
    if (!run) return;
    var items = [], i;
    for (i = 0; i < run.ids.length; i++) {
      if (BY_ID[run.ids[i]]) items.push(BY_ID[run.ids[i]]);
    }
    // 問題を入れ替えたあとなどで見つからないものがあれば、素直に最初からやり直す
    if (items.length !== run.ids.length) {
      var round = run.round;
      store.run = null;
      save(true);
      toast("問題が更新されたため、第" + round + "回を最初から出題します");
      startMock(round);
      return;
    }
    start({
      items: items, ordered: true, mode: "mc", count: items.length,
      isMock: true, mockRound: run.round,
      title: "第" + run.round + "回 模擬試験（つづき）"
    });
    if (!session) return;
  ...
  // 1問も答えていないうちは「途中」と呼ばない。
  // 始めた直後に戻っただけで「0問まで解いた・つづきから」と出るのは、かえって分かりにくい。
  function runProgress() {
    if (!store.run || !store.run.ok.length) return null;
    return { round: store.run.round, done: store.run.ok.length, total: store.run.ids.length };
  }
  ...
  function streakDays() {
    var n = 0, d = new Date();
    // 0 は「模試だけやった日」の印。数ではなく、あるかどうかで判定する。
    if (store.days[todayKey(d)] == null) d.setDate(d.getDate() - 1); // 今日まだなら昨日から
    while (store.days[todayKey(d)] != null) { n++; d.setDate(d.getDate() - 1); }
    return n;
  }
  ...
  function start(opts) {
    ...
    var picked;
    if (opts.ordered) {
      picked = p.slice(0, opts.count || p.length); // つづきから。並べ替えない
    } else if (opts.count === "all") {
  ...
  function renderHome() {
    ...
    $("#goal-headline").textContent =
      ...
      : "仕上げ。本番形式で確認しよう";
    // 数字が3つ（覚えた・目標・出題範囲）あるので、どれが何かを言葉で示す。
    // 「3 / 326（全369問中）」のように並べると、どこと比べた数字か分からなくなる。
    $("#goal-detail").textContent = "覚えた " + g.mastered + "問　／　目標 " + g.goal + "問";
    $("#scope-note").textContent = "出題されるのは、必須①〜⑥と選んだ選択枠1つの " + g.total
      + "問です。アプリには全" + ALL.length + "問が入っていますが、選んでいない選択枠の "
      + (ALL.length - g.total) + "問は出ません。目標の " + g.goal
      + "問は、必須9割・選択7割で本番80点に届く数です。"
      + "上の「達成度」は正答率ではなく目標までの進み具合で、"
      + "「あと一歩」を0.6問、「あやふや」を0.2問と数えるため、覚えた問題数だけの割合より先に進みます。";
    ...
    $("#mock-label").textContent = prog ? "模擬試験（途中）" : "模擬試験";
    $("#mock-desc").textContent = "60問・本番と同じ構成。" + MOCK_PASS
      + "点で合格し、称号がもらえます（「今日の◯問」とは別枠）";
    var nextBadge = badgeFor(round);
    $("#mock-next").textContent = prog
      ? "第" + prog.round + "回を " + prog.done + " / " + prog.total + "問まで解いています。つづきから再開できます"
      : cleared
  ...
  function finish(ok, skipExplain) {
    ...
    if (session.isMock && store.run && store.run.round === session.mockRound) {
      store.run.ok.push(!!ok);
      save();
    }
  ...
  function renderResult() {
    // 最後まで解き切ったので、途中の記録はもう要らない
    if (session.isMock && store.run) { store.run = null; save(true); }
    ...
    $("#result-again").onclick = function () {
      // 同じ回をもう一度。合格して次の回に進んでいても、いま解いた回をやり直す。
      if (session.isMock) { startMock(session.mockRound); return; }
      start({
  ...
  function renderMockList() {
    ...
    mockRounds().forEach(function (round) {
      ...
      right.appendChild(el("span", "chrow__meta",
        midway ? "つづきから" : done ? "受け直す" : "挑戦する"));
      row.appendChild(right);

      row.addEventListener("click", function () {
        if (midway) resumeMock(); else startMock(round);
      });
      list.appendChild(row);
    });

    // 途中のものを捨てて最初からやり直したいとき用
    var restart = $("#mock-restart");
    restart.style.display = prog ? "" : "none";
    restart.textContent = "途中の第" + (prog ? prog.round : "") + "回をやめて、最初から解き直す";
  }
  ...
  function bind() {
    ...
    $("#mock-restart").addEventListener("click", function () {
      var prog = runProgress();
      if (!prog) return;
      store.run = null;
      save(true);
      startMock(prog.round);
    });
