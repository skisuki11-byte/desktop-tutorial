...
// GitHub Pages の公開場所。このリポジトリの Pages は main ブランチのルートを
// 配信しているので、リポジトリ直下の sekaishi/ がそのまま
// https://<ユーザー名>.github.io/<リポジトリ名>/sekaishi/ になる。
const out = path.join(root, "..", "sekaishi");
fs.mkdirSync(out, { recursive: true });
fs.writeFileSync(path.join(out, "index.html"), standalone);
fs.writeFileSync(path.join(out, ".nojekyll"), ""); // Jekyll の変換を通さない

const kb = (s) => Math.round(Buffer.byteLength(s) / 1024) + "KB";
console.log("版:", stamp);
console.log("sekaishi/index.html", kb(standalone), "(GitHub Pages 用)");
