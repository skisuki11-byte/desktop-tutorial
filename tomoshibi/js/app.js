...
(function () {
  ...
  function wire() {
    ...
    $('#btn-store').onclick = function () {
      ...
      var paras = ['写真も動画も記録も、<b>この端末の中だけ</b>に保存しています。' +
        'どこにも送っていないので、外に漏れることはありません。'];
      if (nat) {
        // ネイティブアプリでは追加の説明なし（アプリを消さない限り消えないのは自明のため）
      } else if (i.embedded) {
        paras.push('<b>ただし、いまの開きかたは試し用です。</b>アプリを閉じると、入れたものが消えることがあります。' +
          '（この画面は別のページの中に埋め込まれていて、そこでの保存は一時的なものとして扱われるためです）<br><br>' +
          'ずっと残したいときは、<b>ホーム画面に追加してから開いてください。</b>');
      } else if (!i.idb) {
        paras.push('いまの開きかたでは、写真1枚ぶんしか保存できません。<br>' +
          'ホーム画面に追加してから開くか、SafariやChromeで直接開いてください。');
      } else if (i.durable) {
        paras.push('<b>消されない保存になっています。</b>ブラウザが自動で消すことはありません。');
      } else {
        paras.push('長いあいだ開かないと、ブラウザが自動で消すことがあります。<br>' +
          '<b>ホーム画面に追加しておく</b>と、消されにくくなります。');
      }
      paras.push('端末を変えるときは、設定の「バックアップを書き出す」で持ち出して、新しい端末で読み込ませてください。');
      var body = paras.join('<br><br>');
