...
(function (global) {
  ...
  function verdict(s, l) {
    ...
    if (up(s) && up(l)) return { level: 'good', title: '伸びている',
      summary: '直近28日も、90日で見ても増えています。いま出しているものが当たっている状態です。本数を増やすのがいちばん効きます。' };
    if (up(s) && down(l)) return { level: 'ok', title: '持ち直している',
      summary: '90日では落ちていますが、直近28日は増えています。何かが効き始めています。直近で伸びた動画の作りを次に繰り返してください。' };
    if (down(s) && up(l)) return { level: 'warn', title: '勢いが落ちている',
      summary: '90日で見れば増えていますが、直近28日は減っています。ここで手を打たないと下降に入ります。いちばん気をつけるべき局面です。' };
    if (down(s) && down(l)) return { level: 'bad', title: '下降が続いている',
      summary: '28日も90日も減っています。本数を増やすより先に、なぜ見られなくなったかを1本ずつ確かめる段階です。' };
    if (up(s) || up(l)) return { level: 'ok', title: 'ゆるやかに増えている',
      summary: '大きな動きはありませんが、下がってはいません。次の一手で方向が決まります。' };
    if (down(s) || down(l)) return { level: 'warn', title: 'ゆるやかに落ちている',
      summary: '急な落ち込みではありませんが、じわじわ減っています。原因は1本ではなく作り方の癖にあることが多い局面です。' };
    return { level: 'info', title: '横ばい',
      summary: '28日も90日もほぼ変わっていません。いまのやり方を続けても景色は変わらないので、何か1つ変えて反応を見る段階です。' };
  }
